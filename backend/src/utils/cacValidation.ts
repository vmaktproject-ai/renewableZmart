/**
 * Nigeria CAC (Corporate Affairs Commission) Registration Validation
...
export function normalizeCACNumber(regNumber: string): string {
  ...
  // Match: RC123456, RC 123456, RC-123456
  // Output: RC-123456
...
/**
 * Verify CAC number via CAC Public Search (requires manual verification)
 * Note: CAC does not provide a public API, so this returns instructions
 */
export function getCACVerificationInstructions(regNumber: string): {
  url: string;
  instructions: string[];
} {
  const normalized = normalizeCACNumber(regNumber);
  ...
  return {
    ...
    instructions: [
      ...
      `2. Enter the registration number: ${normalized}`,
      '3. Verify the company name matches the vendor\'s business name',
      '4. Check the registration status is "Active"',
      '5. Confirm the registration date is valid',
      '6. Download the company search result as proof'
    ]
  };
}
 ...
 */
export function formatCACError(result: CACValidationResult): string {
  let message = result.error || 'Invalid CAC registration number';
